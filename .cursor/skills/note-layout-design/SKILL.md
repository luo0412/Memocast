...
description: Memocast 笔记应用 UI 布局设计指南。用于分析、修改或扩展 Memocast 的界面布局，包括三栏布局、分割器、响应式设计、主题系统和组件层级结构。遇到三栏/双栏/单栏切换、分割器、侧栏显隐、分类/标签/日历区域切换、浮动操作栏、大纲抽屉、编辑区容器，或布局样式联动问题时应自动使用。
---
