...
//   注：nice 现已升级为 echo-chant（高亮整行除 @nice 之外的文本为 <mark>），
//       其余 builtin 仍是无运行时副作用的"标记 / 排版 / 引用 / 待办"卡。
//
// === 共享代码 ===
...
// ============================================================================
// 1. nice：高亮行内除 @nice 之外的文本（<mark> 包裹），jQuery 实现
// ============================================================================
// 与 renderer 端的 createNiceAnnoSource 语义对齐；main 进程不执行 handler，
// 但 anno_source 必须保持一致以避免 DB 覆写时漂移。
const createNiceAnnoSource = () => `export default {
  ${banner([
    ...
    '机制：jQuery 直接拿 chantNode 所在 block，contents() 过滤后再 wrapAll 进 <mark>。'
  ])},
  kind: 'echo-chant',
  id: 'nice',
  version: 1,
  ...
  render (context = {}) {
    ...
    return {
      ...
      prompt,
      attrs: { ...attrs, kind: 'echo-chant', id: 'nice', scope: attrs.scope || 'block', inheritFromPrevious: false },
...
// 15 个 echo-chant 内置 id 集中导出，方便外部按 id 查找
