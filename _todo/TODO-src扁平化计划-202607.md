...
| 2026-07-19 | v0.9（css + contextMenu + layouts 拍平） | (1) **用户新指令**：`@src/css 挪到 @src/assets 下面`；`src/css/` 4 文件 → `src/assets/css/`，quasar.conf.js css 入口改成 `{ path: '~/src/assets/css/style.css' }` 绕过 Quasar 强制 `src/css/` 前缀（line 346）；commit `b6dbec4`。(2) **用户新指令**：`@src/contextMenu 转到@src/components 下`；`src/contextMenu/` (4 subdir + utils.js) → `src/components/contextMenu/` 保留 subdir 结构；改 4 .vue + 4 actions.js 的 `src/contextMenu/utils` 引用；git tag `backup-before-contextmenu-to-components-20260719-213100`；commit `019523c`。(3) **用户新指令**：`@src/layouts 挪到@src/components 下面`；`src/layouts/MainLayout.vue` 单文件拍平到 `src/components/MainLayout.vue`（按 §3.4 大胆重构：单文件无需保留 subdir 结构）；改 `src/router/routes.js` line 5 `'layouts/MainLayout.vue'` → `'components/MainLayout.vue'`；git tag `backup-before-layouts-flatten-20260719-214000`；commit `699d554`。3 次拍平都触发 dev server `DONE Compiled successfully`，worktree 持续 clean。 |
